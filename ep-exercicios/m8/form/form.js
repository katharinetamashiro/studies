...
function validatePasswordConfirmation(inputName, input) {
  ...
  var check = isEmpty(input.value) || (document.getElementById('password').value) == (input.value);
  return showOrHideErrorMessage(inputName, check, 'invalid');
}
...
function validateBirthday(inputName, input) {
  ...
  // vazio, ele deve ser considerado válido, pois a outra mensagem de erro
  // (sobre ausência/presença) já será exibida.
  var pattern = isEmpty(input.value) || /^([0-9]{2})\/[0-9]{2}\/([0-9]{4})$/.test(input.value);
  return showOrHideErrorMessage(inputName, pattern, 'invalid');
}
...
function validatePhone(inputName, input) {
  ...
  var teste = isEmpty(input.value) || /^[0-9]{2}9?[0-9]{8}$/.test(input.value);
  return showOrHideErrorMessage(inputName, teste, 'invalid');
}
...
function cleanUp(event) {
  ...
  if (window.confirm('Tem certeza que deseja apagar o formulário?')){
...
/**
 * Função para deixar os checkboxes em itálicos quando selecionados
 */
function italic(){
  var cb = document.querySelectorAll("input[type=checkbox]");
  for (var i = 0; i < cb.length; i++) {
    if (cb[i].checked){
      (cb[i].parentElement).classList.add("selected")
    }
    else {
      (cb[i].parentElement).classList.remove("selected")
    }
  }
}
/**

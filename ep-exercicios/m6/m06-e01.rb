...
class MySQLExercises1
  ...
  def q_products_for_sale
    ...
    <<-SQL
      SELECT COUNT(id) AS num_for_sale
      FROM products 
  ...
  def q_products_discounts
    ...
    <<-SQL
      SELECT id, original_price, (-((price / original_price) - 1)) AS discount
      FROM products;
    SQL
  end
  ...
  def q_products_by_popularity
    ...
    <<-SQL
      SELECT products.id, title, COUNT(sales_products.product_id) AS num_sales
      FROM products
      JOIN sales_products ON products.id=sales_products.product_id
      GROUP BY id
      ORDER BY num_sales DESC, title ASC, products.id ASC;
    SQL
  end
